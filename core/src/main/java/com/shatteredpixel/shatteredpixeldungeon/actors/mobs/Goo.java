...
public class Goo extends Mob {
	...
	protected boolean canAttack( Char enemy ) {
		if (pumpedUp > 0){
			...
			return Dungeon.level.distance(enemy.pos, pos) <= 2
						&& new Ballistica( pos, enemy.pos, Ballistica.PENETRATE_BOLT).collisionPos == enemy.pos
						&& new Ballistica( enemy.pos, pos, Ballistica.PENETRATE_BOLT).collisionPos == pos;
		} else {
	...
	public boolean attack( Char enemy, float dmgMulti, float dmgBonus, float accMulti ) {
		...
		if(pumpedUp==0){
			...
			if(Dungeon.level.distance(pos,Dungeon.hero.pos)<=2||enemy==Dungeon.hero){
				super.attack( Dungeon.hero, dmgMulti, dmgBonus, accMulti );
				Statistics.qualifiedForBossChallengeBadge = false;
			...
			for(Mob m :Dungeon.level.mobs.toArray(new Mob[0])){
				...
				if(canAttack(m)){
					 super.attack(m,dmgMulti, dmgBonus, accMulti);
				}
			}
			result=true;
			pumpedUp = 0;
