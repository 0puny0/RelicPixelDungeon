...
public class Armor extends EquipableItem {
	...
	public void execute(Hero hero, String action) {
		...
		if (action.equals(AC_DETACH) && seal != null&&inlay==Inlay.brokenSeal){
	...
	public float evasionFactor( Char owner, float evasion ){
		...
		if (hasGlyph(Stone.class, owner) && !((Stone)getGlyph(Stone.class, owner)).testingEvasion()){
			return 0;
		}
	...
	public boolean removeCurse(boolean extraEffect) {
		...
		if(hasCurseGlyph()&& extraEffect==true){
			inscribe(null);
		}
	...
	public String info() {
		...
		if (cursed && isEquipped( Dungeon.hero )) {
			...
			if(seal.getGlyph()!=null){
				info += " " +  Messages.capitalize(Messages.get(Armor.class, "seal_inscribed", seal.getGlyph().name()));
			}
	...
	public Glyph getGlyph(Class<?extends Glyph> type, Char owner){
		if(glyph!=null&&glyph.getClass()==type){
			return glyph;
		}
		if (seal!=null&&seal.getGlyph()!=null&&seal.getGlyph().getClass()==type){
