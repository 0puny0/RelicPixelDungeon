...
public class HornOfPlenty extends Artifact {
	...
	public void execute( Hero hero, String action ) {
		...
		if (action.equals(AC_EAT) || action.equals(AC_SNACK)){
			...
			if (!isEquipped(hero)) GLog.i( Messages.get(Artifact.class, "need_to_equip") );
				...
				int satietyPerCharge = (int) (Food.UNIT_ENERGY*3/5f);
				Hunger hunger = Buff.affect(Dungeon.hero, Hunger.class);
				int chargesToUse = Math.max( 1,(Hunger.WELLFED-hunger.hunger()) / satietyPerCharge);
				...
				float eatTime;
				if (action.equals(AC_SNACK)){
					eatTime=Food.TIME_TO_EAT/3;
				} else {
					eatTime=Food.TIME_TO_EAT;
				}
				...
				hero.spend(eatTime);
				Talent.onFoodEaten(hero, eatTime, satietyPerCharge * chargesToUse, this);
	...
	public void gainFoodValue( Food food ){
		if (level() >= 10) return;
		float foodVal=food.energy;
		storedFoodEnergy += food.energy;
		if (storedFoodEnergy >= Food.UNIT_ENERGY*2){
			int upgrades = storedFoodEnergy /( (int)Food.UNIT_ENERGY*2);
			upgrades = Math.min(upgrades, 10 - level());
			upgrade(upgrades);
			storedFoodEnergy -= upgrades * Food.UNIT_ENERGY*2;
			if (level() == 10){
				storedFoodEnergy = 0;
	...
	public class hornRecharge extends ArtifactBuff{
		...
		public void gainCharge(float levelPortion) {
			...
			if (charge < chargeCap) {
				...
				float chargeGain = Food.UNIT_ENERGY*3 * levelPortion * (0.25f + (0.125f*level()));
				chargeGain *= RingOfEnergy.artifactChargeMultiplier(target);
				partialCharge += chargeGain;

				//charge is in increments of 1/5 max hunger value.
				while (partialCharge >= Food.UNIT_ENERGY*3/5) {
					charge++;
					partialCharge -= (Food.UNIT_ENERGY*3/5);
